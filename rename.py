...
def main(argv):
    ...
    if argv:
        if(argv[0][0] is not '-' or len(argv[0]) == 1):
            help_inv(argv[0])
            sys.exit(1)
    ...
    if not filelist:
        if not use_input:
            use_input = './'
        unsorted = os.listdir(use_input)
        filelist = sorted(unsorted, key=len)

    for file in filelist:
        if not (use_input + "/" + file) == "rename.py":
            filename, file_extension = os.path.splitext((use_input + "/" + file))

            sum = hash(use_input + "/" + file, use_hash)

            try:
                print("Trying to rename: " + file)

                if os.path.isfile(use_input + "/" + sum + file_extension):
                    print("file " + sum + file_extension + " already exists")
                    if not (use_input + "/" + file) == (use_input + "/" + sum + file_extension):
                        print("Removing: " + file + " because it is a duplicate")
                        os.remove(use_input + "/" + file)
                elif not sum == "dir":
                    print(use_input + "/" + file + file_extension)
                    print(use_input + "/" + sum + file_extension)
                    os.rename(use_input + "/" + file, use_input + "/" + sum + file_extension)
